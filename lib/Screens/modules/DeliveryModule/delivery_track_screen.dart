...
import 'package:driver_tracking_airport/Screens/modules/DeliveryModule/controller/delivery_controller.dart';
import 'package:driver_tracking_airport/Screens/modules/DeliveryModule/delivery_media_screen.dart';
...
class DeliveryTrackScreen extends StatelessWidget {
  ...
  const DeliveryTrackScreen({super.key, required this.initialJob});

  @override
  Widget build(BuildContext context) {
    final DeliveryController deliveryController = Get.put(DeliveryController());
    deliveryController.setOngoingJob(initialJob);

    return Scaffold(
      appBar: AppBar(
        title: Obx(() {
          final job = deliveryController.ongoingJob.value ?? initialJob;
          final status = job.jobStatus?.toLowerCase();
          if (status == 'ontheway') {
            return const Text('Navigate to Pickup');
          } else if (status == 'started') {
            return const Text('Navigate to Delivery');
          }
          return const Text('Delivery Tracking');
        }),
        actions: [
          IconButton(
      ...
      body: Obx(() {
        ...
        return Column(
          children: [
            ...
            if (deliveryController.isWithinRadius.value &&
                deliveryController.isTimerActive.value)
              _buildTimerBanner(deliveryController),

            // Job Info Section
            Expanded(
              flex: 1,
              child: DeliveryJobInfoSection(controller: deliveryController),
            ),
          ],
        );
      }),
      bottomNavigationBar: Obx(() {
        final job = deliveryController.ongoingJob.value ?? initialJob;
        return _buildBottomActionBar(job, deliveryController);
      }),
    );
  }

  /// Build timer banner widget
  Widget _buildTimerBanner(DeliveryController controller) {
    return Obx(() {
  ...
  Widget _buildBottomActionBar(JobModel job, DeliveryController controller) {
    final status = job.jobStatus?.toLowerCase();
          ...
          child: Obx(() {
            ...
            if (controller.isTimerExpired.value) {
              WidgetsBinding.instance.addPostFrameCallback((_) async {
                ...
                final result = await Get.to<DeliveryMediaResult?>(
                  () => DeliveryMediaScreen(
            ...
            return DeliveryJobInfoSection._buildGradientButton(
              onPressed:
              ...
              WidgetsBinding.instance.addPostFrameCallback((_) async {
                ...
                final result = await Get.to<DeliveryMediaResult?>(
                  () => DeliveryMediaScreen(
                    ...
                    headerSubtitle:
                        'Capture photos and video of the vehicle at delivery location.',
...
class DeliveryJobInfoSection extends StatelessWidget {
  final DeliveryController controller;

  const DeliveryJobInfoSection({super.key, required this.controller});
